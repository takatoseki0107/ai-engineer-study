...
| M5 | カラム間移動・カラム内並び替え | ドラッグ&ドロップでタスクを別カラムへ移動する。同カラム内でも任意の位置へ自由に並び替えられる |
...
| W2 | カラム内一括ソート | カラムヘッダーのセレクターで優先度順・期限順に一括並び替えができる。ソート後もD&Dで自由に順序を変更できる |
...
2. 別カラムの任意の位置にドロップする（挿入ラインで位置を視覚確認できる）
3. タスクのステータスと表示順がAPIを通じてDBに保存され、移動先カラムの指定位置に表示される
```

**カラム内並び替え：**
```
1. ボード画面上のタスクカードをドラッグする
2. 同カラム内の任意の位置にドロップする（挿入ラインで位置を視覚確認できる）
3. タスクの表示順がAPIを通じてDBに保存され、指定した位置に並び替えられる
```
...
**トリガー：** ユーザーが並び替えセレクターで条件を選択する

```
1. ボード画面のカラムヘッダーにある並び替えセレクターで条件（優先度順 / 期限順）を選択する
2. 選択した条件に基づいてカラム内のタスクカードが一括で並び替えられる
3. ソート後はD&Dで任意の順序に自由に変更できる
```
